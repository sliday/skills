...
license: MIT
triggers:
  - "find somewhere near home where I can do this every week"
  - "make this a standing time slot"
  - "find a local studio, room, class, or practice venue"
  - "set up a recurring personal activity"
tools:
  - web
  - browser
  - calendar
...
- one clear venue recommendation near the user's real origin;
...
### 1. Resolve the behavioral goal

Identify what the user actually wants to repeat: solo practice, class attendance, court time, making art, exercise, or another restorative activity. Distinguish:

- **space rental** from lessons, coaching, recording, or performance booking;
- **a recurring ritual** from a one-off visit;
- **intrinsic activity** from a productivity project.

Do not add goals, tracking systems, collaborators, or output obligations unless the user asks. For creative and restorative practices, protect play and aliveness.
...
1. door-to-door friction and walking/transit simplicity;
2. explicit suitability for the user's exact mode (for example, a solo guitarist rather than only full bands);
3. ability to reserve a fixed weekly slot or buy a recurring package;
4. required equipment included and ready to use;
5. booking friction;
6. price;
7. aesthetics and extras.

A slightly more expensive place 15 minutes closer is often the better habit venue. Do not optimize price before adherence.

### 4. Verify first-party details

Prefer official websites, official booking systems, and official social profiles. Verify:

- exact address;
- current published price and minimum duration;
- solo/group eligibility;
- required equipment and whether it is included;
- hours or bookable windows, including weekday/weekend differences;
...
Keep this to one candid sentence unless the user asked for programming. The venue search should not turn into unsolicited fitness coaching.

### 4b. Handle minors and dependent participants separately
...
### 5. Calculate travel and cost explicitly

Use routing or geocoding tools for distance. Label estimates as estimates. Use a calculator/tool for weekly, block, and average monthly cost. Show only the figures needed for the decision.

### 6. Check a sustainable calendar slot

If calendar access exists, inspect several future weeks rather than one date. Prefer a slot that:

- has no recurring conflict across the checked horizon;
- avoids squeezing between meetings;
- includes realistic travel buffer;
- is easy to remember;
- does not cannibalize an established restorative or family ritual.

State the horizon checked and that absence of calendar conflicts is not proof of personal availability.

### 7. Use trial → finite block → review

Default commitment design:

1. one paid trial at the proposed time;
2. if the venue works, reserve the same slot for 6–8 weeks;
3. put the recurring calendar event in place only after venue confirmation;
4. review once at the end of the block: continue, change time/place, or stop.
...
- desired trial date/time;
- desired recurring day/time and block length;
- solo/group format;
- required included equipment;
